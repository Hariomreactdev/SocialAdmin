...
const List = ({
  ...
  const handlerButtonClick = async (action: string, params: any) => {
    switch (action) {
      case 'create':
        navigate(ROUTES.CATEGORY.ADD, { replace: true });
        break;
      case 'edit':
        navigate(ROUTES.CATEGORY.EDIT.replace(':id', params._id), { replace: true });
        break;
      case 'view':
        navigate(ROUTES.CATEGORY.VIEW.replace(':id', params._id), { replace: true });
        break;
      case 'delete':
        try {
          await Http({
