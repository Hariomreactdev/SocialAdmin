...
const getListFunction = async ({
  ...
  const response = await Http(ApiPath.modules.list, {
    page,
    limit,
    sort: sortBy,
    sortOrder: order,
  ...
  const { modules, pagination } = response.data.data;
...
const List = ({
  ...
  const handlerButtonClick = async (action: string, params: any) => {
    switch (action) {
      case 'create':
        navigate(ROUTES.MODULES.ADD, { replace: true });
        break;
      case 'edit':
        navigate(ROUTES.MODULES.EDIT.replace(':id', params._id), { replace: true });
        break;
      case 'view':
        navigate(ROUTES.MODULES.VIEW.replace(':id', params._id), { replace: true });
        break;
      case 'delete':
        // try {
        //   await Http({
  ...
  return (
    <>
      <EntityListView
        title={ROUTES.MODULES.TITLES.LIST}
        data={data}
        display={display}
        isLoading={isLoading}
