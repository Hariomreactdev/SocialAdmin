  ...
  const onSubmit = async (data: Inputs) => {
    try {
      const response = isEditMode
        ? await Http(
            {
              ...ApiPath.roles.update,
              url: ApiPath.roles.update.url.replace(':id', id),
            },
            data,
          )
        : await Http(ApiPath.roles.create, data);
      ...
      const fetchDetails = async () => {
        try {
          const response = await Http(
            {
              ...ApiPath.roles.view,
              url: ApiPath.roles.view.url.replace(':id', id),
            },
            {},
          );
            ...
            <Grid item xs={12}>
              ...
              >
                ...
                >
                  ...
                  <TableBody>
                    {modules.map((module) => {
                      // Find the permission object for this module by module.value
                      const permissionsArray = watch ? watch('permissions') || [] : [];
                      const permissionObj = permissionsArray.find(
                        (p: PermissionType) => p.moduleName === module.value,
                      );
                          ...
                          {['create', 'read', 'update', 'delete'].map((perm) => (
                            <TableCell key={perm}>
                              <Checkbox
                                checked={
                                  permissionObj
                                    ? permissionObj.actions[perm as keyof PermissionActions] ===
                                      true
                                    : false
                                }
                                onChange={(e) => {
                                  const currentPermissions = watch
                                    ? watch('permissions') || []
                                    : [];
...
export default withFormHOC<Inputs>({
  ...
  routes: {
    list: ROUTES.ROLES.LIST,
    listTitle: ROUTES.ROLES.TITLES.LIST,
    addTitle: ROUTES.ROLES.TITLES.ADD,
    editTitle: ROUTES.ROLES.TITLES.EDIT,
